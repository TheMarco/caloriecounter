    ...
    var body: some View {
        List {
            ...
            Section {
                ...
                Text("Food descriptions and photos are processed by OpenAI to estimate nutrition. Under OpenAI's API terms this data isn't used to train their models, and because the app has no account and sends nothing that identifies you, these requests can't be tied back to you. Your log, weights, and targets never leave the device — no tracking, no analytics.")
            }
            .font(.subheadline)
            ...
            Section {
                Label("Barcode scans are looked up in Open Food Facts (Open Database License).",
                      systemImage: "barcode")
                    .font(.subheadline)
                ...
                Text("Barcode nutrition comes from the Open Food Facts database, made available under the Open Database License (ODbL). Calorie and macro estimates for typed, spoken, and photographed food are generated by OpenAI and are approximate — always check and adjust them before saving.")
            }
